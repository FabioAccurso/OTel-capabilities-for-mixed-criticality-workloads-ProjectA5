...
/************************ OTEL - Includes and global variables ************************/
...
namespace zipkin_exporter = opentelemetry::exporter::zipkin;
...
/****************************** OTEL - Global main span *******************************/
// main() has various exit points and I need to end the span from all of them
#if (RTAPP_TRACE_LEVEL > 0)
opentelemetry::v1::nostd::shared_ptr<opentelemetry::v1::trace::Span> main_span;
#endif
/**************************************************************************************/
...
/* OTEL - Same as InitTracer(), but spans go to a file inside the run's log directory instead
 * of stdout. Used by the DoE "file" arm, which isolates serialization + local write from the
 * network path of the Zipkin exporter.
 *
 * The path is derived from opts.logdir (already parsed when this runs), NOT from an
 * environment variable: test.sh launches rt-app through `sudo cset shield --exec`, and sudo
 * resets the environment, so an env-based path would be silently ignored and the spans would
 * end up on stdout without any error.
 *
 * span_file is static on purpose: OStreamSpanExporter keeps a *reference* to the stream, so
 * the stream must outlive the TracerProvider. A local variable would leave it dangling. */
void InitTracerFile(const char *logdir) {

	static std::ofstream span_file;

	std::string path = (logdir ? std::string(logdir) : std::string(".")) + "/spans.log";
	span_file.open(path, std::ios::out | std::ios::trunc);
	if (!span_file.is_open()) {
		log_error("Cannot open OTel span file %s", path.c_str());
		exit(EXIT_FAILURE);
	}
	log_notice("OTel spans -> %s", path.c_str());

	// 1. Resource
	resource::ResourceAttributes attributes = { {"service.name", "rt-app_file"} };
	auto resource = resource::Resource::Create(attributes);

	// 2. Exporter: ostream exporter bound to the file stream above
	auto exporter = trace_exporter::OStreamSpanExporterFactory::Create(span_file);

	// 3. Processor
	// NOTE: same #if chain as InitTracer() and InitTracerZipkin(), with the same parameters,
	// on purpose: the three initializers must differ ONLY in the exporter, otherwise the DoE
	// arms (none / file / net) would not be comparable.
	#if (RTAPP_PROCESSOR_TYPE == 0)
	// Batch
	trace_sdk::BatchSpanProcessorOptions batch_options{};
	batch_options.max_queue_size = 2048;
...
/* OTEL - utility function to access Tracer */
auto get_tracer() {
	return opentelemetry::trace::Provider::GetTracerProvider()->GetTracer("rt-app-tracer");
}
#endif
/**************************************************************************************/

static ftrace_data_t ft_data = {
	.tracefs = (char *)TRACEFS_PATH,
	.marker_fd = -1,
};
...
/********************** Helper functions to handler cgroupfs **************************/
#if (RTAPP_CGROUP_ENABLED == 1)
std::string read_cgroup_file(const std::string& path) {
	...
	// Read file content removing the final newline
...
void write_cgroup_file(const std::string& path, const std::string& value) {
	...
	// Write value in file
	file << value << std::endl;
	if (file.fail()) throw std::runtime_error("Kernel rejected the write operation to: " + path);
}

void enter_cgroup() {
	const std::string cgroup_path = "/sys/fs/cgroup/rt-app-cgroup/";
	// Write rt-app PID in cgroup.procs
	pid_t rtapp_pid = getpid();
	write_cgroup_file(cgroup_path + "cgroup.procs", std::to_string(rtapp_pid));
	// Read cpuset.cpus.effective and write the content into cpuset.cpus.exclusive,
	// then check if it has been written into cpuset.cpus.exclusive.effective
	std::string cpus_exclusive = read_cgroup_file(cgroup_path + "cpuset.cpus.effective");
	write_cgroup_file(cgroup_path + "cpuset.cpus.exclusive", cpus_exclusive);
	std::string cpus_exclusive_effective = read_cgroup_file(cgroup_path + "cpuset.cpus.exclusive.effective");
	if (cpus_exclusive != cpus_exclusive_effective) {
		std::cerr << "Error: cpus.exclusive != cpus.exclusive.effective" << "\n";
		exit(EXIT_FAILURE);
	}
	// Write "isolated" (or "root") into cpuset.cpus.partition, then check if it has been written
	write_cgroup_file(cgroup_path + "cpuset.cpus.partition", "isolated");
	std::string isolated_effective = read_cgroup_file(cgroup_path + "cpuset.cpus.partition");
	if (isolated_effective != "isolated") {
		std::cerr << "Error: cpuset.cpus.partition not 'isolated'" << "\n";
		exit(EXIT_FAILURE);
	}
}

void exit_cgroup() {
	// Before exiting rt-app, restore the state of rt-app-cgroup prior to rt-app execution
	// If we don't do this, the cgroup will be an isolated root partition and next rt-app
	// execution won't be able to correctly set its affinity
	const std::string cgroup_path = "/sys/fs/cgroup/rt-app-cgroup/";
	// Write "member" into cpuset.cpus.partition, then check if it has been written
	write_cgroup_file(cgroup_path + "cpuset.cpus.partition", "member");
	std::string isolated_effective = read_cgroup_file(cgroup_path + "cpuset.cpus.partition");
	if (isolated_effective != "member") {
		std::cerr << "Error: cpuset.cpus.partition not 'member'" << "\n";
		exit(EXIT_FAILURE);
	}
	// Write "" into cpuset.cpus.exclusive then read cpuset.cpus.exclusive.effective
	write_cgroup_file(cgroup_path + "cpuset.cpus.exclusive", "");
	std::string cpus_exclusive_effective = read_cgroup_file(cgroup_path + "cpuset.cpus.exclusive.effective");
	if (cpus_exclusive_effective != "") {
		std::cerr << "Error: cpus.exclusive != cpus.exclusive.effective" << "\n";
		exit(EXIT_FAILURE);
	}
}
...
static void thread_data_set_unique_name(thread_data_t *tdata, int nforks)
{
	...
	char *unique_name = static_cast<char *>(malloc(string_size));
	if (unique_name) {
		if (tdata->forked) {
...
static int thread_data_create_unique_resources(thread_data_t *tdata, const thread_data_t *td)
{
	...
	tdata->local_resources = static_cast<rtapp_resources_t *>(malloc(local_size));
...
static int create_thread(const thread_data_t *td, int index, int forked, int nforks)
{
	...
	tdata = static_cast<thread_data_t *>(malloc(sizeof(thread_data_t)));
	if (!tdata) {
	...
	/************************** OTEL - Initialize thread span pointer *************************/
	// malloc() and memcpy() copy a thread_data_t structure, to which we added a span pointer
	// that is a C++ object which we need to correctly initialize
	#if (RTAPP_TRACE_LEVEL > 0)
	new (&tdata->span) opentelemetry::nostd::shared_ptr<opentelemetry::trace::Span>(nullptr);
	#endif
	/******************************************************************************************/
	...
	case rtapp_fork:
		{
			...
			threads = static_cast<pthread_data_t *>(realloc(threads, nthreads * sizeof(*threads)));
...
static void __shutdown(bool force_terminate)
{
	...
	/* Termination forced by signals (like CTRL+C) or because use case duration expired. Cancel threads */
	if (force_terminate) {
		continue_running = 0;	// __shutdown(true) called both by end of duration and by signals (like CTRL+C). continue_running ensures threads are cancelled only once

		pthread_mutex_lock(&fork_mutex);

		for (i = 0; i < running_threads; i++) {
			/****************** OTEL - Handle thread span forced termination *****************/
			// If use case duration expires or termination signals (e.g. CTRL+C)
			#if (RTAPP_TRACE_LEVEL > 0)
			if (threads[i].data->span) {
				threads[i].data->span->SetAttribute("cancellation", "force_terminate");
                if (threads[i].data->span->IsRecording()) {
					threads[i].data->span->End();
				}
				// Call the destructor
			    threads[i].data->span.~shared_ptr();

			    // Set to nullptr so no other function tries to access it
	...
	/***************************** CGROUP - Unset rt-app cgroups ******************************/
	#if (RTAPP_CGROUP_ENABLED == 1)
	exit_cgroup();
	#endif
	/******************************************************************************************/
	
	/**************** OTEL - End main span and Cleanup tracing (success) **************/
	#if (RTAPP_TRACE_LEVEL > 0)
	if (main_span) {
		main_span->SetAttribute("shutdown", "graceful");
		main_span->AddEvent("graceful-shutdown");
		if (main_span->IsRecording())
			main_span->End();
		main_span = nullptr;
	}
	CleanupTracer();
...
static int create_cpuset_str(cpuset_data_t *cpu_data)
{
	...
	cpu_data->cpuset_str = static_cast<char *>(malloc(size_needed));
...
static void __set_thread_policy_priority(thread_data_t *data,
					 sched_data_t *sched_data)
{
...
void *thread_body(void *arg)
{
	...
	/* Init timing buffer */
	if (opts.logsize > 0) {
		timings = static_cast<timing_point_t *>(malloc(opts.logsize));
		/*
	...
	/***************************** OTEL - Create Thread Span *******************************/
	#if (RTAPP_TRACE_LEVEL > 0)
	auto tracer = get_tracer();
	opentelemetry::trace::StartSpanOptions span_opts;
	span_opts.parent = main_span->GetContext();
	data->span = tracer->StartSpan(std::string(data->name), span_opts);	
    auto scope = tracer->WithActiveSpan(data->span); // maybe useless

	// Set thread span attributes (a lot of checks to avoid dereferencing null pointers)
	if (data->span) {
		// configuration attributes
		data->span->SetAttribute("config.index", 					(int)data->ind);
		data->span->SetAttribute("config.lock_pages", 				(int)data->lock_pages);
		data->span->SetAttribute("config.duration", 				(int)data->duration);
		data->span->SetAttribute("config.loop", 					(int)data->loop);
		data->span->SetAttribute("config.nphases", 					(int)data->nphases);
		data->span->SetAttribute("config.delay", 					(int)data->delay);
		data->span->SetAttribute("config.forked", 					(int)data->forked);
		data->span->SetAttribute("config.num_instances", 			(int)data->num_instances);
	
		// cpuset attributes
		data->span->SetAttribute("config.name", 					data->name ? std::string(data->name) : "null");
		if (data->cpu_data.cpuset_str)
			data->span->SetAttribute("config.cpuset", std::string(data->cpu_data.cpuset_str));
		if (data->numa_data.numaset_str)
			data->span->SetAttribute("config.numa_nodes", std::string(data->numa_data.numaset_str));
		
		// scheduling attributes
		if (data->sched_data) {
			data->span->SetAttribute("config.sched_data.policy", 		policy_to_string(data->sched_data->policy));
			data->span->SetAttribute("config.sched_data.priority", 		(int)data->sched_data->prio);
			if (data->sched_data && (data->sched_data->policy == 4)) { // If task SCHED_DEADLINE
				data->span->SetAttribute("config.sched_data.period", 	(int)data->sched_data->period);
				data->span->SetAttribute("config.sched_data.runtime", 	(int)data->sched_data->runtime);
				data->span->SetAttribute("config.sched_data.deadline",	(int)data->sched_data->deadline);
			}
			data->span->SetAttribute("config.sched_data.util_max", 		(int)data->sched_data->util_max);
			data->span->SetAttribute("config.sched_data.util_min", 		(int)data->sched_data->util_min);
		}
	} /* end thread span attributes */
	#endif
	/***************************************************************************************/
	...
	/***************************** OTEL - Create inner spans *******************************/
	// Inner spans handlers (they need to be visible out of the loop)
	// Trace levels are designed to be used like this:
	//		RTAPP_TRACE_OFF 	= 0		No tracing
	//		RTAPP_TRACE_TOP 	= 1		Only main and thread spans
	//		RTAPP_TRACE_PHASE 	= 2		+ Thread loops and phases
	//		RTAPP_TRACE_ALL 	= 3		+ Phase loops (very high granularity)
	#if (RTAPP_TRACE_LEVEL > 1)
	opentelemetry::nostd::shared_ptr<opentelemetry::trace::Span> thread_loop_span;
	thread_loop_span = nullptr;
	opentelemetry::nostd::shared_ptr<opentelemetry::trace::Span> phase_span;
	phase_span = nullptr;
	#endif
	#if (RTAPP_TRACE_LEVEL > 2)
	opentelemetry::nostd::shared_ptr<opentelemetry::trace::Span> phase_loop_span;
	phase_loop_span = nullptr;
	#endif
	/***************************************************************************************/

	/* The following is executed for each phase. */
	while (continue_running && thread_loop != data->loop) {		// thread_loop iterations (continue_running set to zero by forced termination (duration expired or CTRL+C))
		
		/*************************** OTEL - Inner spans lifecycle *****************************/
		#if (RTAPP_TRACE_LEVEL > 1)
		if (data->span) {
			// Thread Loop Span
			if (phase == 0 && phase_loop == 0) {
				if (thread_loop_span) thread_loop_span->End();
				opentelemetry::trace::StartSpanOptions t_opts;
				t_opts.parent = data->span->GetContext();			// Set thread span as parent
				thread_loop_span = tracer->StartSpan(
					"thread_loop[" + std::to_string(thread_loop) + "]",
					t_opts);										// Start new span
			}
			// Phase Span
			if (phase_loop == 0) {
				if (phase_span) phase_span->End();	
				opentelemetry::trace::StartSpanOptions p_opts;
				p_opts.parent = thread_loop_span->GetContext();
				phase_span = tracer->StartSpan(
					"phase[" + std::to_string(phase) + "]",
					p_opts);
			}
			// Phase Loop Span
			#if (RTAPP_TRACE_LEVEL > 2)
			if (phase_loop_span) phase_loop_span->End();
			opentelemetry::trace::StartSpanOptions p_loop_opts;
			//////////////////////////////////////////////////			To get each phase loop on a different trace (to test Sampler) uncommend this and comment the next
//			opentelemetry::context::Context empty_context;
//			auto token = opentelemetry::context::RuntimeContext::Attach(empty_context);
//			phase_loop_span = tracer->StartSpan(
//				"phase_loop[" + std::to_string(phase_loop) + "]"
//			);
			//////////////////////////////////////////////////
			p_loop_opts.parent = phase_span->GetContext();
			phase_loop_span = tracer->StartSpan(
				"phase_loop[" + std::to_string(phase_loop) + "]",
				p_loop_opts);

			auto phase_loop_scope = tracer->WithActiveSpan(phase_loop_span);
			#endif
		}
		#endif
		/*************************************************************************************/
		...
		/*************************** OTEL - Phase loop spans attributes *****************************/
		#if (RTAPP_TRACE_LEVEL > 2)
		if (phase_loop_span) {	
			// current cpuset attributes
			if (data->curr_cpu_data)
				phase_loop_span->SetAttribute("cpuset.cpus", 		(data->curr_cpu_data->cpuset_str) ? std::string(data->curr_cpu_data->cpuset_str) : "null");
			if (data->curr_numa_data)
				phase_loop_span->SetAttribute("cpuset.numa-nodes",	(data->curr_numa_data->numaset_str) ? std::string(data->curr_numa_data->numaset_str) : "null");

			// current scheduling attributes
			if (data->curr_sched_data) {
				phase_loop_span->SetAttribute("sched_data.policy", 			policy_to_string(data->curr_sched_data->policy));
				phase_loop_span->SetAttribute("sched_data.priority", 		(int)data->curr_sched_data->prio);
				if (data->curr_sched_data->policy == 4) {		// If task SCHED_DEADLINE
					phase_loop_span->SetAttribute("sched_data.period",		(int)data->curr_sched_data->period);
					phase_loop_span->SetAttribute("sched_data.runtime",		(int)data->curr_sched_data->runtime);
					phase_loop_span->SetAttribute("sched_data.deadline",	(int)data->curr_sched_data->deadline);					
				}
				phase_loop_span->SetAttribute("sched_data.util_max", 		(int)data->curr_sched_data->util_max);
				phase_loop_span->SetAttribute("sched_data.util_min", 		(int)data->curr_sched_data->util_min);
			}
			
			// latency metrics
			if (curr_timing) {
				phase_loop_span->SetAttribute("stats.period", 	curr_timing->period);
				phase_loop_span->SetAttribute("stats.run", 		curr_timing->duration);
				phase_loop_span->SetAttribute("stats.wu_lat", 	curr_timing->wu_latency);
				phase_loop_span->SetAttribute("stats.slack", 	curr_timing->slack);
				phase_loop_span->SetAttribute("stats.c_period", curr_timing->c_period);
				phase_loop_span->SetAttribute("stats.c_run",	curr_timing->c_duration);
			}
			
			// current phase events
			if (pdata && pdata->events) {
				for (int i = 0; i < pdata->nbevents; i++) {
					event_data_t *event = &pdata->events[i];
					phase_loop_span->SetAttribute("event[" + std::to_string(i) + "].name", std::string(event->name));
					phase_loop_span->SetAttribute("event[" + std::to_string(i) + "].duration", event->duration);
				}
			}
	...
	/**************************** OTEL - End inner spans *******************************/
	// End spans from the last iteration
	#if (RTAPP_TRACE_LEVEL > 2)
	if (phase_loop_span) phase_loop_span->End();
	#endif
	#if (RTAPP_TRACE_LEVEL > 1)
...
int main(int argc, char* argv[])
{
	...
	// Initialize thread span pointer (see similar operation in create_thread())
	// opts are initialized through malloc() while parsing the command line, which doesn't
	// correctly initialize the thread span pointers, which are C++ objects
	for (int i = 0; i < opts.num_tasks; i++) {
	...
	InitTracer();
	#else
	InitTracerFile(opts.logdir);
	#endif

	// Create main span
	auto tracer = get_tracer();
    main_span = tracer->StartSpan("main");
	auto scope = tracer->WithActiveSpan(main_span);

	// Set main span attributes
	if (main_span) {
		main_span->SetAttribute("config.lock_pages", opts.lock_pages);
		main_span->SetAttribute("config.nthreads", 			opts.nthreads);
		main_span->SetAttribute("config.num_tasks", 			opts.num_tasks);
		main_span->SetAttribute("config.policy", 				(int)opts.policy);
		main_span->SetAttribute("config.duration", 			opts.duration);
		main_span->SetAttribute("config.logdir", 				opts.logdir ? std::string(opts.logdir) : "null");
		main_span->SetAttribute("config.logbasename", 			opts.logbasename ? std::string(opts.logbasename) : "null");
		main_span->SetAttribute("config.logsize", 				opts.logsize);
		main_span->SetAttribute("config.gnuplot", 				opts.gnuplot);
		main_span->SetAttribute("config.calib_cpu", 			opts.calib_cpu);
		main_span->SetAttribute("config.calib_ns_per_loop",	opts.calib_ns_per_loop);
		main_span->SetAttribute("config.pi_enabled", 			opts.pi_enabled);
		main_span->SetAttribute("config.die_on_dmiss", 		opts.die_on_dmiss);
		main_span->SetAttribute("config.mem_buffer_size", 		opts.mem_buffer_size);
		main_span->SetAttribute("config.io_device", 			opts.io_device ? std::string(opts.io_device) : "null");
		main_span->SetAttribute("config.cumulative_slack", 	opts.cumulative_slack);
		// main_span->SetAttribute("threads_data", opts.threads_data);
		// main_span->SetAttribute("resources", opts.resources);
	}
	...
	threads = static_cast<pthread_data_t *>(malloc(nthreads * sizeof(*threads)));
	if (!threads) {
	...
	/***************************** OTEL - Create calibration span ****************************/
    #if (RTAPP_TRACE_LEVEL > 0)
	auto calibration_span = tracer->StartSpan("calibration");
	auto calibration_scope = tracer->WithActiveSpan(calibration_span);
	#endif
	/*****************************************************************************************/
	...
	/***************************** OTEL - End calibration span *******************************/
	#if (RTAPP_TRACE_LEVEL > 0)
	calibration_span->End();
	#endif
	/*****************************************************************************************/
	...
	/***************************** CGROUP - Set rt-app cgroups ********************************/
	// Bypassing rt-app cgroups logic with a simple implementation for the experiment
	#if (RTAPP_CGROUP_ENABLED == 1)
	log_notice("ENTER CGROUP");
	enter_cgroup();
	#endif
	/*****************************************************************************************/
...
exit_err:
	...
	/**************** OTEL - End main span and Cleanup tracing (failure) **************/
	#if (RTAPP_TRACE_LEVEL > 0)
	if (main_span) {
		main_span->SetAttribute("shutdown", "error");
		main_span->AddEvent("error-exit");
		if (main_span->IsRecording())
			main_span->End();
		main_span = nullptr;
	}
