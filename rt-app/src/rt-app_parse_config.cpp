...
static void init_membuf_resource(rtapp_resource_t *data, const rtapp_options_t *opts)
{
	...
	data->res.buf.ptr = static_cast<char *>(malloc(opts->mem_buffer_size));
		...
		int idx, const rtapp_options_t *opts)
{
	...
	data->type = static_cast<resource_t>(type);
		  ...
		  rtapp_resource_t *data, const rtapp_options_t *opts)
{
	...
	/* resource type */
	resource_to_string(static_cast<resource_t>(0), def_type);
...
add_resource_data(const char *name, int type, rtapp_resources_t **resources_table, rtapp_options_t *opts)
{
	...
	*resources_table = static_cast<rtapp_resources_t *>(realloc(*resources_table, size));
...
parse_resources(struct json_object *resources, rtapp_options_t *opts)
{
	 ...
	 */
	opts->resources = static_cast<rtapp_resources_t *>(malloc(sizeof(rtapp_resources_t)));
	if (!opts->resources) {
	...
	if (json_object_is_type(resources, json_type_object)) {
		...
		opts->resources = static_cast<rtapp_resources_t *>(realloc(opts->resources, size));
		if (!opts->resources) {
				log_error("Failed to allocate memory for resources");
				exit(EXIT_FAILURE);
		}
...
static void parse_cpuset_data(struct json_object *obj, cpuset_data_t *data)
{
	...
	if (cpuset_obj) {
		...
		data->cpuset = static_cast<cpu_set_t *>(malloc(data->cpusetsize));
...
static sched_data_t *parse_sched_data(struct json_object *obj, int def_policy)
{
	...
	/* Get default policy */
	def_str_policy = policy_to_string(static_cast<policy_t>(def_policy));
	...
	if (tmp_data.prio != THREAD_PRIORITY_UNCHANGED ||
		...
		new_data = static_cast<sched_data_t *>(malloc(sizeof(sched_data_t)));
		  ...
		  phase_data_t *data, thread_data_t *tdata, rtapp_options_t *opts)
{
	...
	data->events = static_cast<event_data_t *>(malloc(data->nbevents * sizeof(event_data_t)));
		  ...
		  thread_data_t *data, rtapp_options_t *opts)
{
	 ...
	 * parsing resources table or when parsing task's event
	 */
	data->local_resources = static_cast<rtapp_resources_t *>(malloc(sizeof(rtapp_resources_t)));
	...
	if (phases_obj) {
		...
		data->phases = static_cast<phase_data_t *>(malloc(sizeof(phase_data_t) * data->nphases));
		foreach(phases_obj, entry, key, val, idx) {
...
parse_tasks(struct json_object *tasks, rtapp_options_t *opts)
{
	 ...
	 */
	opts->threads_data = static_cast<thread_data_t *>(malloc(sizeof(thread_data_t) * opts->num_tasks));
	foreach (tasks, entry, key, val, idx)
