...
policy_to_string(policy_t policy)
{
	switch (policy) {
		case other:
			return (char *)"SCHED_OTHER";
		case idle:
			return (char *)"SCHED_IDLE";
		case rr:
			return (char *)"SCHED_RR";
		case fifo:
			return (char *)"SCHED_FIFO";
		case deadline:
			return (char *)"SCHED_DEADLINE";
		default:
			return NULL;
	}
	return NULL;
...
void ftrace_write(int mark_fd, const char *fmt, ...)
{
	...
	if ((tmp = static_cast<char *>(malloc(size))) == NULL) {
	...
	while(1) {
		...
		if ((ntmp = static_cast<char *>(realloc(tmp, size))) == NULL) {
			free(tmp);
