...
static void initialize_taskgroups(void)
{
	...
	ctrl.tg_array = static_cast<taskgroup_data_t *>(malloc(size));
	if (!ctrl.tg_array) {
		perror("malloc");
		exit(EXIT_FAILURE);
	}
...
taskgroup_data_t *alloc_taskgroup(size_t size)
{
	...
	tg->name = static_cast<char *>(malloc(size));
	if (!tg->name) {
		perror("malloc");
		return NULL;
	}
...
static int cgroup_get_cpu_controller_mount_point(void)
{
	...
	while ( (ent = getmntent(mounts)) ) {
		...
		ctrl.mount_point = static_cast<char *>(malloc(strlen(ent->mnt_dir) + 1));
		if (!ctrl.mount_point) {
			perror("malloc");
			break;
		}
...
static int cgroup_mkdir(const char *name, int *offset)
{
	...
	*offset = -1;
	...
	path = static_cast<char *>(malloc(size));
	if (!path) {
		perror("malloc");
		ret = -1;
		goto error;
...
static int cgroup_attach_task(char *name)
{
	...
	file = (char *)(strcmp(name, "/") ? "/tasks" : "tasks");
